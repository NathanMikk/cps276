...
class Validation{
	...
	private function address($value){
		$match = preg_match('/^\d+\s+[a-z0-9\s.-]{1,50}$/i', $value);
		return $this->setError($match);
	}

	private function city($value){
		$match = preg_match('/^[a-zA-Z]+(?:[\s-][a-zA-Z]+)*$/', $value);
		return $this->setError($match);
	}
	...
	private function dob($value){
		$match = preg_match('/^(0[1-9]|1[0-2])\/(0[1-9]|[12][0-9]|3[01])\/(19|20)\d{2}$/', $value);
		return $this->setError($match);
	}

	//password checks for at least one letter, one number, one special character, and is between 5 and 20 characters in length.
	private function password($value){
		$match = preg_match('/^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{5,20}$/', $value);
		return $this->setError($match);
	}
